...
class SogCLR(torch.optim.Optimizer):
    r"""
    Stochastic Optimization for sovling :obj:`~libauc.losses.GCLoss`. For each iteration **t**, the key updates for **SogCLR** are sumarized as follows:
        
        1. Initialize :math:`\tau, \mathbf w_0, \mathbf u_0= \mathbf 0`
        2. For :math:`t=1, \ldots, T`:
        3. :math:`\hspace{5mm}` Draw a batch of :math:`B` samples
        4. :math:`\hspace{5mm}` Compute :math:`g(\mathbf{w}_t ; \mathbf{x}_i, \mathbf{B}^-_i) = \sum_{\mathbf{x}_j \sim \mathbf{B}^-_i} \exp (h_{\mathbf{w}}(\mathbf{x}_i)^{\top} h_{\mathbf{w}}(\mathbf{x}_j) / \tau)`
        5. :math:`\hspace{5mm}` Compute :math:`\mathbf{u}_{i, t}=(1-\gamma) \mathbf{u}_{i, t-1}+\gamma \frac{1}{2|\mathbf{B}_i|} g(\mathbf{w}_t ; \mathbf{x}_i, \mathbf{B}^-_i)`
        6. :math:`\hspace{5mm}` Compute the gradient estimator :math:`\mathbf{m}_t = -\frac{1}{B} \sum_{\mathbf{x}_i \in \mathbf{B}} \nabla\left(h_\mathbf{w}\left(\mathbf{x}_i\right)^{\top} h_\mathbf{w}\left(\mathbf{x}_i^+\right)\right) +\frac{\tau}{\mathbf{u}_{i,t} } \nabla g\left(\mathbf{w};\mathbf{x}_i;  \mathbf{B}_i^{-}\right)`
        7. :math:`\hspace{5mm}` Update model :math:`\mathbf{w_t}` by *Momemtum* or *Adam optimzier*
            
    For more details, please refer to `Provable Stochastic Optimization for Global Contrastive Learning: Small Batch Does Not Harm Performance <https://arxiv.org/abs/2202.12387>`__.
